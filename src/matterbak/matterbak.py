...
class Init:
    ...
    def __init__(self):
        ...
        try:
            ...
            if self.options.output_zip is None:
                self.options.output_zip = pl.Path(
                    f'matterbak_{self.calling_username}.zip')
            ...
            print("JSON structure of a config file broken "
                  "(note that a common cause for a misleading error message "
                  f"is a comma after the last element of a container): {ex}")
            sys.exit(1)
    ...
    def _parse_command_line(self):
        ...
        parser.add_argument(
            "--credentials", type=pl.Path, default=pl.Path('credentials.json'),
            help="json file containing user name, password and server URL, "
            "default = %(default)s")
        parser.add_argument(
            "--channels", type=pl.Path, default=pl.Path('channels.json'),
            help="json file listing all channels to backup, "
            "default = %(default)s")
        parser.add_argument(
            "-d", "--data-dir", type=pl.Path, default=default_data_dir,
            help="Dir to store downloaded data in, "
            "absolute or relative to current dir, default = %(default)s")
        parser.add_argument(
            "-o", "--output-zip", type=pl.Path, default=None,
            help="zip file to write, default is 'matterbak_<user>.zip'")
        parser.add_argument(
            "--update-old-posts", action="store_true", default=False,
            help="Update also old posts in case they changed since last update")
        parser.add_argument(
            "--skip-direct", action="store_true", default=False,
            help="skip direct channels")
        parser.add_argument(
            "--skip-groups", action="store_true", default=False,
            help="skip group channels")
        parser.add_argument(
            "--skip-teams", action="store_true", default=False,
            help="skip team channels")
        parser.add_argument(
            "--skip-users", action="store_true", default=False,
            help="Skip storing personal user data "
            "(includes --skip-user-images)")
        parser.add_argument(
            "--skip-user-images", action="store_true", default=False,
            help="Skip storing user images")
        parser.add_argument(
            "--skip-emojis", action="store_true", default=False,
            help="Skip storing custom emojis")
        parser.add_argument(
            "--rate-limit", type=float, default=10,
            help="Max API calls per second. Default: %(default)s. "
            "Set to 0 to disable.")
        parser.add_argument(
            "--initial-jitter", type=float, default=0,
            help="Random delay in seconds at script start. "
            "Default: %(default)s.")
        parser.add_argument(
...
def backup_direct_channels(init):
    ...
    Stores all direct channels listed under the key 'direct' in the
...
def is_backup_group_channel(member_usernames, config):
    """
    Return True if a group channel with member_usernames is configured to be
    backed up
    ...
    under 'subset' is a match if the configured names are a subset of
    member_usernames.
    """
...
def backup_group_channels(init):
    ...
    Stores all group channels configured under the key 'group' in the channels
    config file.
...
def select_channels_by_names(all_channels, team_config, names_key):
    ...
    return: set of channels with the names given as <name_key> in the
            channels_config
    """
    names = set(team_config.get(names_key, []))
    channels = {c for c in all_channels if (
        c['display_name'] in names) or (c['name'] in names)}
...
def backup_team_channels(init):
    ...
    for team_name, team_config in init.channels_config.get('teams', {}).items():
        ...
        if not team_channels_to_backup:
            team_channels_to_backup = team_channels.copy()

        exclude_channels = select_channels_by_names(
            team_channels, team_config, 'exclude')
        team_channels_to_backup -= exclude_channels

        print_channels(team_channels_to_backup, "to backup")
        ...
        dump.dump_content(
            team_dir, members, id_=team['id'], name=f"{team_name}{dump.FILENAME_SEPARATOR}members")
...
def backup_custom_emojis(init):
    ...
    for emoji in init.matter.get_list_of_custom_emojis():
        ...
        skip_existing = False
        old_emoji = dump.dump_content(
            emojis_dir, emoji, name=emoji['name'], return_old_content=True)
        if old_emoji and (emoji['update_at'] <= old_emoji['update_at']):
            skip_existing = True

        image_loader = functools.partial(
            init.matter.get_custom_emoji_image, emoji['id'])
        dump.dump_image(emojis_dir, emoji['id'], image_loader,
                        label=emoji['name'], skip_existing=skip_existing)
    print()
