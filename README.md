...
We do a safe interruption handling: Writes of images and JSON files are protected against `Ctrl+C` (SIGINT) and `kill` (SIGTERM) signals. The program delays interruptions while writing to prevent data corruption. After writing finishes, normal interruption behavior resumes -- you can safely stop the program with `Ctrl+C` or `kill` at any time.
